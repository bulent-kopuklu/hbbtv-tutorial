...
appli_name = "encom demo hbbtv application" #application name
appli_root = "http:/172.17.0.2/hbbtv-app" #URL base of transport_protocol_descriptor
...
out = open("output/nit.sec", "wb")
out.write(nit.pack())
out.close
out = open("output/nit.sec", "wb") # python  flush bug
out.close
os.system('opencaster/bin/sec2ts 16 < output/nit.sec > output/nit.ts')

out = open("output/sdt.sec", "wb")
out.write(sdt.pack())
out.close
out = open("output/sdt.sec", "wb") # python   flush bug
out.close
os.system('opencaster/bin/sec2ts 17 < output/sdt.sec > output/sdt.ts')

out = open("output/pat.sec", "wb")
out.write(pat.pack())
out.close
out = open("output/pat.sec", "wb") # python   flush bug
out.close
os.system('opencaster/bin/sec2ts 0 < output/pat.sec > output/pat.ts')

out = open("output/pmt.sec", "wb")
out.write(pmt.pack())
out.close
out = open("output/pmt.sec", "wb") # python   flush bug
out.close
os.system('opencaster/bin/sec2ts ' + str(pmt1_pid) + ' < output/pmt.sec > output/pmt.ts')

out = open("output/ait.sec", "wb")
out.write(ait.pack())
out.close
out = open("output/ait.sec", "wb") # python   flush bug
out.close
os.system('opencaster/bin/sec2ts ' + str(ait1_pid) + ' < output/ait.sec > output/ait.ts')

os.system('rm output/*.sec') # deleting of the section files.
